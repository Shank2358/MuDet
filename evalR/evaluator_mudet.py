...
from evalR.voc_evalnew import voc_eval
...
class Evaluator(object):
    ...
    def APs_voc_Single(self, img_ind):
        img_path = os.path.join(self.val_data_path, 'rgb', img_ind + '.jpg')  # 路径+JPEG+文件名############png
        img_rgb = cv2.imread(img_path)
        img_dsm = cv2.imread(img_path.replace('rgb', 'dsm'))
        #print(img_dsm.shape)
        #img = img_rgb#np.concatenate((img_rgb, img_dsm), axis = -1) #
        bboxes_prd = self.get_bbox(img_rgb, img_dsm, self.multi_test, self.flip_test)
        for bbox in bboxes_prd:
            ...
            if int(class_ind) == 0: color = (255, 0, 0)
    ...
    def get_bbox(self, img, img_dsm, multi_test=False, flip_test=False):
        ...
        if multi_test:
            ...
            for test_input_size in test_input_sizes:
                ...
                bboxes_list.append(self.__predict(img, img_dsm, test_input_size, valid_scale))
                if flip_test:
                    bboxes_flip = self.__predict(img[:, ::-1], img_dsm[:, ::-1], test_input_size, valid_scale)
                    bboxes_flip[:, [0, 2]] = img.shape[1] - bboxes_flip[:, [2, 0]]
                    bboxes_list.append(bboxes_flip)
            bboxes = np.row_stack(bboxes_list)
        else:
            bboxes = self.__predict(img, img_dsm, self.val_shape, (0, np.inf))
    ...
    def __predict(self, img, img_dsm, test_shape, valid_scale):
        org_img = np.copy(img)
        org_h, org_w, _ = org_img.shape
        img = self.__get_img_tensor(img, test_shape).to(self.device)
        img_dsm = self.__get_img_tensor(img_dsm, test_shape).to(self.device)
        self.model.eval()
        with torch.no_grad():
            start_time = current_milli_time()
            _, p_d = self.model(img, img_dsm)
    ...
    def convert_pred(self, pred_bbox, test_input_size, org_img_shape, valid_scale):
        ...
        scores = torch.pow(pred_conf, 1-self.alpha) * torch.pow(pred_prob[torch.arange(len(pred_xyxy)), classes], self.alpha)
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        ...
        for i, cls in enumerate(self.classes):
            R, P, AP = voc_eval(filename, annopath, imagesetfile, cls, cachedir, iou_thresh, use_07_metric)  # 调用voc_eval.py的函数进行计算
            APs[cls] = AP
            Recalls[cls] = R
            Precisions[cls] = P
